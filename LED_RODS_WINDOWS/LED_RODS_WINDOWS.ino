...
#define NUM_LEDS  60
#define NUM_LEDS_2 96
#define NUM_LEDS_3 144
...
LaunchKey_IndexList[14]=27;
LaunchKey_IndexList[15]=28;
...
{
    int gaussianSpeed= map(_CCnotes[12].CCvalue,0,127,1,20);
  ...
  */
        ...
        case 43:  //  for(int i=0;i<NUM_LEDS;i++)leds[i]=CHSV(354,230,255); fadeValue=100;
                 // break;
                  ...
                  for(int i=0;i<NUM_LEDS;i++)
                    { for(int s=0;s<NUM_STRIPS;s++)
                       ledstrips[s][i]=CHSV(354,230,255); 
                    }
                  fadeValue=map(_CCnotes[15].CCvalue,0,127,20,100);
                    break;
        ...
        case 50: for(int i=0;i<NUM_LEDS;i++)
         ...
         case 52: for(int i=0;i<NUM_LEDS;i++)
           ...
           case 55: for(int i=0;i<NUM_LEDS;i++)
            ...
            case 56:
                    ...
                    { int hue=random8(0,255);
                      ...
                      for(int i=0;i<NUM_LEDS;i++)
                       ledstrips[s][i]=CHSV(hue,255,255) ;
                    }
                   fadeValue=100;
                    break;
          ...
          case 58:    counter2=counter2+1;
           ...
           case 59: for(int i=0;i<NUM_LEDS;i++)
                    {
                       ledstrips[3][i]=CHSV(6,230,255); 
                    }
                  fadeValue=map(_CCnotes[15].CCvalue,0,127,20,100);
                     ...
                     // current_palette=passionfruit_gp;
                      ...
                      for(int s=0;s<NUM_STRIPS;s++)
                      {
                        for(int i=0;i<NUM_LEDS;i++)
                        {
                          ...
                          ledstrips[s][i]=ColorFromPalette(current_palette,map(int(i+counter2+15*s)%NUM_LEDS,0,NUM_LEDS,0,255),255,LINEARBLEND);
                          
                          ledstrips[s][i]=ColorFromPalette(current_palette,i+noise,255,LINEARBLEND);
...
void floatingOrbs(CCnote _CCnotes[])  //group of consecutive LEDS move about freely across the strip
{
  /* 
 for(int i=0;i<60;i++){
...
brightness=0;
    ...
    */
    int delta_fade=15;
    int orb_brightness=map(_CCnotes[14].CCvalue,0,127,0,255);
...
void ChasingOrbs(CCnote _CCnotes[],int mp_index)
{
    int delta_fade=8;
    int orb_brightness=map(_CCnotes[14].CCvalue,0,127,0,255);
    int orb_midpoint[4]={30,30,30,30};
